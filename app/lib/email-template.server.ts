...
export const CANVAS = "#F6F6F8";
/** One font stack for every template — email clients have no web fonts. */
export const FONT =
...
/**
 * The MetaVault email chrome: navy header, coloured accent rule, white card,
 * grey footer. Every template we send goes through here so they stay one family.
 *
 * `content` and `footer` are raw HTML — callers are responsible for escaping
 * anything merchant-supplied before it lands in them.
 */
...
<div style="display:none;font-size:1px;color:${CANVAS};max-height:0;overflow:hidden;">${escapeHtml(
    preheader,
  )}</div>
      ...
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="width:100%;max-width:560px;background:#FFFFFF;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(16,24,46,0.08);">
        <tr>
          <td style="padding:20px 32px;background:${NAVY};">
            <span style="font:600 17px ${FONT};color:#FFFFFF;letter-spacing:-0.2px;">
...
export function buildHtml(input: TemplateInput): string {
  ...
  const statsRows = (input.stats ?? [])
    .map(
      (s) => `
              <tr>
                <td style="padding:8px 0;border-bottom:1px solid ${BORDER};font:14px ${FONT};color:${MUTED};">${escapeHtml(s.label)}</td>
                <td align="right" style="padding:8px 0;border-bottom:1px solid ${BORDER};font:600 14px ${FONT};color:${INK};">${escapeHtml(s.value)}</td>
              </tr>`,
    )
    .join("");
  ...
  const errorBlock =
    input.status === "failed" && input.error
      ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
              <tr>
                <td style="padding:14px 16px;background:#FEF2F2;border-left:3px solid #EF4444;border-radius:6px;font:13px ${MONO};color:#991B1B;word-break:break-word;">${escapeHtml(
                  input.error,
                )}</td>
              </tr>
            </table>`
      ...
      ? `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 12px;">
              <tr>
                <td style="border-radius:8px;background:${INDIGO};">
                  <a href="${escapeHtml(input.downloadUrl)}" style="display:inline-block;padding:12px 24px;font:600 15px ${FONT};color:#FFFFFF;text-decoration:none;border-radius:8px;">${escapeHtml(
                    cta,
                  )}</a>
                </td>
              </tr>
            ...
            ${
              input.linkValidForDays
                ? `<p style="margin:0 0 8px;font:13px ${FONT};color:${MUTED};">This link works for ${input.linkValidForDays} days.</p>`
                : ""
            }`
      : "";
  ...
  return emailShell({
    ...
    accent,
    content: `<h1 style="margin:0 0 12px;font:600 21px ${FONT};color:${INK};letter-spacing:-0.3px;">${escapeHtml(
      heading,
    )}</h1>
            <p style="margin:0 0 24px;font:15px/1.6 ${FONT};color:${MUTED};">${escapeHtml(
              intro,
            )}</p>
            ${statsTable}
            ${errorBlock}
            ${button}`,
    footer: `<p style="margin:0 0 6px;font:13px/1.5 ${FONT};color:${MUTED};">
              You're getting this because job notifications are on for your store.
            </p>
            <p style="margin:0;font:13px/1.5 ${FONT};color:${MUTED};">
              Turn them off in MetaVault → Settings → Email notifications.
            </p>`,
  });
}
